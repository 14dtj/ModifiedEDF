...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
		...
		if (((EDF_DATA*)OSTCBCur->OSTCBExtPtr)->comp_time == 0) {
			...
			taskQueue[level].elements->deadline = ((EDF_DATA*)OSTCBCur->OSTCBExtPtr)->ddl;
			taskQueue[level].elements->id = OSTCBCur->OSTCBId;
			taskQueue[level].elements->importance = ((EDF_DATA*)OSTCBCur->OSTCBExtPtr)->importance;
			taskQueue[level].elements->prio = OSTCBCur->OSTCBPrio;
			printf("\n%d\tComplete\t%d\t%d", OSTimeGet(),OSTCBCur->OSTCBId, getEDFQueNextId());
		}
		else {
			if (OSTCBCur->OSTCBId != getEDFQueNextId()) {
				APP_TRACE("\n%d\tPreempt\t\t%d\t%d", OSTimeGet(),OSTCBCur->OSTCBId, getEDFQueNextId());
			}
		}
		if (OSTCBCur->OSTCBDly == 0) {
			qd.deadline = ((EDF_DATA*)OSTCBCur->OSTCBExtPtr)->ddl;
			qd.prio = OSTCBCur->OSTCBPrio;
			update(level);
		}
		OS_EXIT_CRITICAL();
    }
...
static void OS_SchedEDF(void) {
	...
	while (p_current->OSTCBPrio != OS_TASK_IDLE_PRIO) {
		...
		if (p_current->OSTCBPrio<60 && p_current->OSTCBDly == 0 && ((EDF_DATA*)p_current->OSTCBExtPtr)->comp_time > 0) {
...
static INT32U getEDFNextID(void) {
	...
	edf_ptcb = OSTCBList;
	OS_ENTER_CRITICAL();
	while (p_current->OSTCBPrio<60 && p_current->OSTCBPrio != OS_TASK_IDLE_PRIO) {
